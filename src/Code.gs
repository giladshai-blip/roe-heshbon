...
function syncRiseUpV5() {
  ...
  const metrics={inserted:0,updated:0,unchanged:0,duplicates:0,months:[],riseupLastUpdatedAt:'',budgetUpdated:false,balanceBefore:'',balanceAfter:'',reconciliation:null};
  try{
    ...
    let all=[];
    metrics.months.forEach(function(month){const r=riseupGet_('/api/external/transactions?cashflowMonth='+encodeURIComponent(month),pat);const txs=requireTransactions_(r);all=all.concat(withCashflowMonth_(txs,month));});
...
function sheetSafeText_(value){const s=String(value??'');return /^[=+\-@]/.test(s)?"'"+s:s;}
