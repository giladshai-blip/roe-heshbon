/**
 * ============================================================
 * רואה חשבון — Core | Release dev-3.0.1 | Legacy Build V5.10.1
 * ============================================================
 * קובץ מלא להחלפת Code.gs ב-Google Apps Script.
 *
 * עיקרי V5.10.1:
 * - גיליון "תנועות" נשמר אוטומטית בסדר כרונולוגי יורד אחרי Setup וסנכרון.
 ...
 * - בדיקות Runtime כוללות כעת parity של KPI בין Core לתצוגות ובדיקת סדר כרונולוגי.
 * ============================================================
 */

const V56 = {
  RELEASE_VERSION: 'dev-3.0.1',
  LEGACY_BUILD_ID: 'V5.10.1',
...
function setupV56() {
  ...
  try {
    ...
    sortTransactionsChronologically_();
    refreshDuplicateFormulas_();
    setConfigParam_('גרסת מערכת', V56.RELEASE_VERSION, '', 'Core ' + V56.RELEASE_VERSION);
    setConfigParam_('Legacy Build ID — Core', V56.LEGACY_BUILD_ID, '', 'Compatibility/build identifier only');
    ...
    logSync_({action:V56.RELEASE_VERSION + ' Setup',status:health.status,records:0,message:'התקנת Core ' + V56.RELEASE_VERSION + ' + בדיקות שלמות',syncState:'SETUP',health:health.summary});
    getSpreadsheet_().toast(health.status==='SUCCESS'?V56.RELEASE_VERSION+' הותקן ונבדק בהצלחה':V56.RELEASE_VERSION+' הותקן — יש לעיין בדוח הבדיקה','רואה חשבון',8);
    return health;
  } finally { lock.releaseLock(); }
}

function setupV5101() { return setupV56(); }
...
function syncRiseUpV5() {
  ...
  const lock=LockService.getScriptLock();
  if(!lock.tryLock(10000)){safeLogSync_({action:'RiseUp Sync '+V56.RELEASE_VERSION,status:'SKIPPED',records:0,message:'סנכרון אחר כבר פעיל',syncState:'LOCKED'});return null;}
  ...
  try{
    ...
    setConfigParam_('תאריך רענון אחרון',new Date(),'','RiseUp Sync '+V56.RELEASE_VERSION);
    setConfigParam_('מצב hash',metrics.budgetUpdated?'UPDATED':'SKIPPED_UNCHANGED','','Budget hash');
    applyModelIntegrityFixes_();ensureAutomaticBankBalanceFormula_();sortTransactionsChronologically_();refreshDuplicateFormulas_();SpreadsheetApp.flush();
    ...
    const health=healthCheckV56_();
    logSync_({time:startedAt,action:'RiseUp Sync '+V56.RELEASE_VERSION,status:health.status,records:all.length,message:'חודשים: '+metrics.months.join(', ')+' | חדשות: '+metrics.inserted+' | עודכנו: '+metrics.updated+' | ללא שינוי: '+metrics.unchanged+' | כפילויות API: '+metrics.duplicates+' | תכנון נסרק: '+metrics.reconciliation.eligible+' | התאמות בטוחות: '+metrics.reconciliation.matched+' | לבדיקה: '+metrics.reconciliation.review,tokenRef:metrics.tokenRef,riseupLastUpdatedAt:metrics.riseupLastUpdatedAt,syncState:metrics.budgetUpdated?'UPDATED':'SKIPPED_UNCHANGED',durationMs:Date.now()-startMs,inserted:metrics.inserted,updated:metrics.updated,duplicates:metrics.duplicates,balanceBefore:metrics.balanceBefore,balanceAfter:metrics.balanceAfter,health:health.summary});
    showSyncToast_(metrics,health);return {success:health.ok,metrics:metrics,health:health};
  }catch(e){safeLogSync_({time:startedAt,action:'RiseUp Sync '+V56.RELEASE_VERSION,status:'ERROR',records:0,message:String(e.message||e),durationMs:Date.now()-startMs,health:'ERROR'});throw e;}
  finally{lock.releaseLock();}
}
...
function syncRiseUpHistory12MonthsV5(){
  ...
  try{
    ...
    const result=upsertTransactions_(all);applyModelIntegrityFixes_();ensureAutomaticBankBalanceFormula_();SpreadsheetApp.flush();sortTransactionsChronologically_();refreshDuplicateFormulas_();
    refreshFinancialViewsV510_();
    SpreadsheetApp.flush();
    const health=healthCheckV56_();
    logSync_({action:'RiseUp History 12M '+V56.RELEASE_VERSION,status:health.status,records:all.length,message:'נטענו 12 חודשים | '+details.join(' | '),syncState:'HISTORY_BACKFILL',durationMs:Date.now()-start,inserted:result.inserted,updated:result.updated,duplicates:result.duplicates,health:health.summary});
    getSpreadsheet_().toast('היסטוריית 12 חודשים סונכרנה','רואה חשבון',7);return result;
  }catch(e){safeLogSync_({action:'RiseUp History 12M '+V56.RELEASE_VERSION,status:'ERROR',records:0,message:String(e.message||e),durationMs:Date.now()-start});throw e;}finally{lock.releaseLock();}
}
...
function refreshForecastsV5(){const lock=LockService.getScriptLock();if(!lock.tryLock(30000))throw new Error('סנכרון פעיל');try{validateRequiredSheets_();validateSyncSchema_();applyModelIntegrityFixes_();ensureAutomaticBankBalanceFormula_();sortTransactionsChronologically_();refreshDuplicateFormulas_();SpreadsheetApp.flush();refreshFinancialViewsV510_();SpreadsheetApp.flush();const h=healthCheckV56_();logSync_({action:'Forecast Refresh '+V56.RELEASE_VERSION,status:h.status,records:0,message:'רענון הפניות ללא שינוי הנחות',syncState:'FORECAST_REFRESH',health:h.summary});getSpreadsheet_().toast(h.summary,'רואה חשבון',8);return h;}finally{lock.releaseLock();}}

function healthCheckV5(){const h=healthCheckV56_();SpreadsheetApp.getUi().alert('Health Check '+V56.RELEASE_VERSION,h.summary,SpreadsheetApp.getUi().ButtonSet.OK);return h;}
...
function runRuntimeSelfTestV57(){
  ...
  test('סדר כרונולוגי בתנועות',function(){if(!transactionsAreChronologicallySorted_())throw new Error('גיליון תנועות אינו מסודר מהחדש לישן');return 'תנועות מסודרות מהחדש לישן';});
  ...
  safeLogSync_({action:'Runtime Self Test '+V56.RELEASE_VERSION,status:failed.length?'ERROR':'SUCCESS',records:tests.length,message:summary,syncState:'SELF_TEST',health:summary});
  SpreadsheetApp.getUi().alert('בדיקות Runtime '+V56.RELEASE_VERSION,summary,SpreadsheetApp.getUi().ButtonSet.OK);
...
function healthCheckV56_(){
  ...
  const sync=getConfigParam_('תאריך רענון אחרון');if(!isValidDate_(sync)||sync.getTime()>Date.now())warnings.push('מועד סנכרון אחרון חסר או לא תקין');else if(Date.now()-sync.getTime()>V56.SYNC_STALE_HOURS*3600000)warnings.push('הסנכרון האחרון ישן מ-'+V56.SYNC_STALE_HOURS+' שעות');if(getConfigParam_('גרסת מערכת')!==V56.RELEASE_VERSION)warnings.push('גרסת Core המותקנת בהגדרות אינה '+V56.RELEASE_VERSION);if(getConfigParam_('Legacy Build ID — Core')!==V56.LEGACY_BUILD_ID)warnings.push('Legacy Build ID של Core המותקן בהגדרות אינו '+V56.LEGACY_BUILD_ID);if(typeof DASHBOARD_V56!=='undefined'&&getConfigParam_('גרסת דשבורד')!==DASHBOARD_V56.VERSION)warnings.push('גרסת הדשבורד דורשת התקנה: '+DASHBOARD_V56.VERSION);
...
function sortTransactionsChronologically_(){const sh=getSheet_('TRANSACTIONS'),lr=sh.getLastRow();if(lr<3)return;const width=Math.max(sh.getLastColumn(),22);ensureGridSize_(sh,lr,width);sh.getRange(2,1,lr-1,width).sort([{column:2,ascending:false},{column:21,ascending:false},{column:1,ascending:true}]);}
function transactionsAreChronologicallySorted_(){const sh=getSheet_('TRANSACTIONS'),lr=sh.getLastRow();if(lr<3)return true;const dates=sh.getRange(2,2,lr-1,1).getValues();let previous=Infinity;for(let i=0;i<dates.length;i++){const d=dates[i][0];if(!isValidDate_(d))continue;const t=d.getTime();if(t>previous)return false;previous=t;}return true;}
...
function getFinancialSnapshotV510_(){const cash=getSheet_('CASHFLOW'),annual=getSheet_('ANNUAL_CASHFLOW');const cashRows=cash.getRange(2,1,Math.max(1,cash.getLastRow()-1),7).getValues(),annualRows=annual.getRange(16,1,Math.max(1,annual.getLastRow()-15),9).getValues();const forecast=forecastWindow_(cashRows,annualRows,new Date()),end=endOfCashflow_(cashRows),currentBalance=numberOrNaN_(getConfigParam_('יתרת עו״ש מחושבת אוטומטית')),checkingFrame=numberOrNaN_(getConfigParam_('מסגרת עו״ש מאומתת'));let breachDate='',breachAmount=NaN;if(isFinite(checkingFrame)&&checkingFrame>0&&forecast.covered===30){for(let i=0;i<forecast.values.length;i++){const value=forecast.values[i];const date=addDaysKey_(dayKey_(new Date()),i);if(value<-checkingFrame){breachDate=date;breachAmount=value;break;}}}return{version:V56.RELEASE_VERSION,calculatedAt:new Date(),currentBalance:currentBalance,monthEnd:end.value,monthEndDate:end.day,low30:forecast.minimum,low30Date:forecast.minimumDate,coveredDays:forecast.covered,checkingFrame:checkingFrame,remainingFrameAtLow:isFinite(checkingFrame)&&checkingFrame>0&&isFinite(forecast.minimum)?checkingFrame+forecast.minimum:NaN,breachDate:breachDate,breachAmount:breachAmount,lastSync:getConfigParam_('תאריך רענון אחרון'),bankVerifiedAt:getConfigParam_('תאריך ושעת יתרת עו״ש'),budget:getBudgetMetricsV510_(),credit:getCreditMetricsV510_(),safetyBuffer:getSafetyBufferMetricsV510_()};}
function getFinancialSnapshotV58_(){return getFinancialSnapshotV510_();}

function refreshBudgetViewKpisV510_(optionalSnapshot){const s=optionalSnapshot||getFinancialSnapshotV510_(),b=s.budget,sh=getSheet_('MONTHLY_BUDGET');[['A5',b.actualIncome],['C5',b.actualExpenses],['E5',b.variableBudget],['G5',b.variableRemaining],['A8',b.actualFixed],['C8',b.actualVariable],['E8',b.fixedBudget],['G8',b.actualBalance]].forEach(function(x){sh.getRange(x[0]).setValue(x[1]).setNumberFormat('#,##0.00 ₪').setNote('KPI מחושב ב-Core '+V56.RELEASE_VERSION+' | חודש '+b.month);});sh.getRange('A9').setValue(b.status).setNote('סטטוס מחושב ב-Core '+V56.RELEASE_VERSION);return b;}
