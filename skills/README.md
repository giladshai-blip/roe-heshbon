...
- V1.3: נוספו ארבעה Skills על בסיס דפוסי שימוש חוזרים בפרויקט; נדרש Regression לפני קידום ל־`main`.
- דוחות הבדיקה נמצאים תחת `tests/`.

החבילה נשמרת ב־`dev` עד השלמת Regression וקריאה חוזרת; לאחר מכן הסוכן רשאי לקדם את רכיבי ה־Skills הרלוונטיים ל־`main` באופן אוטונומי.
