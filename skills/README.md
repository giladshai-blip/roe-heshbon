...
גרסה נוכחית: **V1.4 ב־main**. מועמדת ב־`dev`: **V1.5** עם `financial-model-architect` לריכוז בעלות על חישובים ו־KPI.
...
- `financial-model-architect` — מיפוי בעלות על חישובים ו־KPI, איתור drift בין קוד לגיליון, Shadow/Parity/Cutover וריכוז המערכת למודל של "מוח אחד, הרבה תצוגות".
...
9. שינוי ארכיטקטורת חישובים חייב לעבור Map → Owner → Shadow → Parity → Cutover → Deprecate, ללא Big Bang.
...
- "איפה החישוב מפוזר?", "מי הבעלים של KPI?", "איך עוברים למוח אחד?" → `financial-model-architect` עם `financial-system-auditor` כתומך.
...
- V1.5: מועמדת ב־`dev`; בדיקות ארכיטקטורה יירשמו בדוח ייעודי לפני קידום.
