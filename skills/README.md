...
גרסה נוכחית: **V1.4-dev ב־dev**. גרסה יציבה אחרונה ב־`main`: **V1.3**.
...
- V1.4-dev: נוספו שני Skills והורחב ה־Router; טרם קודמה ל־`main` עד השלמת regression וקריאה חוזרת.
- דוחות הבדיקה נמצאים תחת `tests/`.

V1.3 קודמה סלקטיבית ל־`main` לאחר Regression וקריאה חוזרת של קובצי ה־Skill, ה־Router והתיעוד.