...
- קישורי GitHub שמסרה דב הם חלק ממצב הפרויקט הידוע לגבי; לפני שגבי אומר שמידע טכני אינו ידוע/לא זמין, עליו לבדוק את `main`, את `release.json` ואת קישורי GitHub הקנוניים הרלוונטיים.
...
- בכל תשובת סיום של משימת קוד/סקריפט/ריליס דב מצרפת **קישור GitHub ישיר** לקוד הקנוני ששונה; אם נוצרו PR או commit רלוונטיים, מצרפת גם אותם.
...
- DEV שמסיימת משימת קוד/סקריפט/ריליס ללא קישור GitHub ישיר לקוד ששונה → FAIL.
