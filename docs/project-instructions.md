...
- בקשת DEV/קוד → טען `agents/dev-engineering-agent/AGENT.md` ואת `RUNTIME.md` לפי הצורך; אל תטען מידע פיננסי אלא אם הוא דרוש לבדיקה.
...
- דב רשאית לבצע autonomously פעולות פיתוח הפיכות ב־`dev`: קוד, refactor, tests, diagnostics, bugfixes, documentation ו-release metadata.
